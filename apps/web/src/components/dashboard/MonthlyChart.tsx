...
const CustomTooltip = ({
  ...
  const expenses = payload.find(
    (p: { dataKey?: string; value: number }) => p.dataKey === "expensesNeg",
  );
  const savings = payload.find((p: { dataKey?: string; value: number }) => p.dataKey === "savings");
  return (
    <div className="min-w-[170px] rounded-lg border border-[#1E2B42] bg-[#131C2E] px-3 py-2.5 text-[12px] shadow-xl">
      <div className="mb-2 font-medium text-[#A8B4CC]">{label}</div>
      {income && (
        <div className="mb-1 flex items-center gap-2">
          ...
          <span className="ml-auto font-medium text-white">
            {formatCurrencyCompact(income.value)}
          </span>
        </div>
      )}
      {expenses && (
        <div className="mb-1 flex items-center gap-2">
          ...
          <span className="ml-auto font-medium text-white">
            {formatCurrencyCompact(Math.abs(expenses.value))}
          </span>
        </div>
      )}
      {savings && (
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto font-medium text-white">
            {formatCurrencyCompact(savings.value)}
          </span>
        </div>
      )}
    </div>
  ...
  return (
    <div>
      <div className="mb-3 flex items-center gap-4 text-[11px]">
        ...
        <div className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-full" style={{ background: SAVINGS_COLOR }} />
