...
Moniqo supports signing in with a third-party identity provider as an alternative and complement to password-based login. A user may link multiple providers to a single Moniqo account.
...
| `provider_subject` | String | Yes | The provider's stable, unique subject identifier (OIDC `sub` claim) |
...
`:provider` is `google`, resolved dynamically from the provider registry, never hardcoded in a handler. A provider that is not registered/recognized (unconfigured, e.g. missing credentials) behaves identically to an unknown one.
...
When the redirect callback completes, the resulting Moniqo user is resolved in this order:
...
**An unverified provider email is always rejected outright** (the callback fails generically) — there is no partial or pending path for third-party signups, since Moniqo has no channel to verify an email the identity provider itself won't vouch for. This is a hard rule, checked before any account lookup.
